...
for (i in 1:10) {
  ...
  data <- point_creator(n = 3500, sd_x = 5, sd_y = 5, colors = colors)
  ...
  output <- ggplot() + 
    geom_point(data = tibble(x = 0, y = 0), aes(x,y), size = 120, color = "black") + #rand_color(1)
    ...
    geom_point(data = data, mapping = aes(x,y, size = size / 20, color = color)) + 
